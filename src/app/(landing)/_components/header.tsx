  ...
  return (
    <header className="px-2 py-4 lg:py-6">
      <div className="container flex items-center gap-2 p-0">
        ...
        <Link className="flex items-center justify-center px-3 text-xl font-medium" href="/">
          {/* <RocketIcon className="mr-2 h-5 w-5" /> {APP_TITLE} */}
          <Image alt="Logo" src="/logo.png" height={200} width={200} className="h-7 w-7 " />{" "}
          {/* {APP_TITLE} */}
        </Link>
        <nav className="ml-5 hidden gap-4 sm:gap-6 md:flex">
          {routes.map(({ name, href }) => (
            <Link
              key={name}
